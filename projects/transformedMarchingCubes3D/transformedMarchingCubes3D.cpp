...
void marchTranslations(const VEC3F& direction, const string& descriptor)
{
  ...
  {
    for (int i = 0; i < topOriginal.roots().size(); i++)
      optimize3D.top().rootsMutable()[i] = topOriginal.roots()[i] + x * qDelta;
    for (int i = 0; i < bottomOriginal.roots().size(); i++)
      optimize3D.bottom().rootsMutable()[i] = bottomOriginal.roots()[i] + x * qDelta;
...
int main(int argc, char* argv[])
{
  ...
  if (argc != 9)
  {
    cout << " USAGE: " << argv[0] << " <optimize 3D file> <res> <max iterations> <output prefix> <transform> <min> <max> <steps>" << endl;
  ...
  else if (transform.compare("translationz") == 0)
  {
