...
void OBJ::ComputeVertexNormals()
{
	...
	for (unsigned int i = 0; i < faces.size (); i++)
	{
        ...
        VEC3 scaled = normal;
        scaled *= angle;
				normals[faces[i].vertices[j]] -= scaled;
			}
		}
	}
	for (unsigned int i = 0; i < normals.size (); i++)
...
float OBJ::bruteForceDistance(float* point)
{
  ...
  for (unsigned int x = 0; x < faces.size(); x++)
  {
    double dist = pointFaceDistanceSq(faces[x], vertex);
...
// triangle
//////////////////////////////////////////////////////////////////////
float OBJ::distance(const float* point) const
{
  ...
  for (int z = zStart; z < zEnd; z++)
    for (int y = yStart; y < yEnd; y++)
      for (int x = xStart; x < xEnd; x++)
      {
        ...
        map<int, vector<int> >::const_iterator iter = _accelGrid.find(index);
        if (iter == _accelGrid.end())
          continue;

        const vector<int>& bucket = iter->second;
        ...
        for (unsigned int i = 0; i < bucket.size(); i++)
        {
          ...
          double dist = sqrt(pointFaceDistanceSq(faces[faceIndex], vertex));
          if (dist < minDistance) minDistance = dist;
        }
      }
  return minDistance;
...
//////////////////////////////////////////////////////////////////////
bool OBJ::intersect_triangle(float orig[3], float dir[3], int faceIndex) const
{
...
//////////////////////////////////////////////////////////////////////
bool OBJ::isOccupied(double* point)
{
  /*
  float rotated[] = {static_cast<float>(point[0]), 
...
// intersection test
//////////////////////////////////////////////////////////////////////
bool OBJ::inside(float* point) const
{
  float rotated[] = {static_cast<float>(point[0]), 
                     static_cast<float>(point[1]), 
                     static_cast<float>(point[2])};
  ...
  for (int x = 0; x < intPoint[0]; x++)
  {
    ...
    //const vector<int>& faces = _accelGrid[index];
    map<int, vector<int> >::const_iterator iter = _accelGrid.find(index);
    if (iter == _accelGrid.end())
      continue;

    const vector<int>& faces = iter->second;
...
void OBJ::createAccelGrid()
{
  ...
  // for each Face
  cout << " Creating acceleration grid ... "; flush(cout);
  for (unsigned int i = 0; i < faces.size(); i++)
  {
    if (faces[i].vertices.size() != 3) continue;
...
{
  TIMER functionTimer(__FUNCTION__);
  cout << " Building distance grid ..."; flush(cout);
    ...
    for (int z = (int)(floor(zMin * _zResDist)); z <= (int)(ceil(zMax * _zResDist)); z++)
      for (int y = (int)(floor(yMin * _yResDist)); y <= (int)(ceil(yMax * _yResDist)); y++)
        for (int x = (int)(floor(xMin * _xResDist)); x <= (int)(ceil(xMax * _xResDist)); x++)
        {
          ...
          double distanceSq = pointFaceDistanceSq(faces[i], point);

          map<int,double>::iterator finder;
...
//////////////////////////////////////////////////////////////////////////////
double OBJ::fastDistToClosestFace(int vertexID)
{
  map<int,double>::iterator finder;
...
//----------------------------------------------------------------------------
//////////////////////////////////////////////////////////////////////////////
double OBJ::pointFaceDistanceSq(const Face& f, const VEC3F& point) const
{
...
void OBJ::draw()
{
  ...
  for(unsigned int i = 0; i < faces.size(); i++)
  {
    vector<int>& indices = faces[i].vertices;

    vector<VEC3> verts;
    verts.push_back(vertices[indices[0]]);
    verts.push_back(vertices[indices[1]]);
    verts.push_back(vertices[indices[2]]);
...
void OBJ::drawFiltered()
{
  ...
  for(unsigned int i = 0; i < faces.size(); i++)
  {
    ...
    double currentMaxLength = triangle.maxEdgeLength();
...
///////////////////////////////////////////////////////////////////////
void OBJ::writePBRT(const char* filename)
{
...
///////////////////////////////////////////////////////////////////////
// write out XY coordinates as UV coordinates
///////////////////////////////////////////////////////////////////////
void OBJ::writeXYtoUV(const char* filename)
{
  FILE* file = fopen(filename, "w");
